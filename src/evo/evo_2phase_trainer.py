...
def _build_evo_optimizer(optimizer_name: str, pop_size: int, problem: Problem, args, de_param_control):
    """Pymoo DE or PSO, matching evo_trainer.setup."""
    ...
    raise ValueError(f"Invalid optimizer: {optimizer_name}, choices: de, pso")
...
def main(args):
    ...
    phase1_adapter = get_adapter("full", model=model, args=args)

    num_dims = phase1_adapter.num_dims

    phase1_xl = -1.0 if args.phase1_pop_init_lb is None else float(args.phase1_pop_init_lb)
    phase1_xu = 1.0 if args.phase1_pop_init_ub is None else float(args.phase1_pop_init_ub)
    problem = Problem(
        n_var=num_dims,
        n_obj=1,
        n_constr=0,
    ...
    optimizer = _build_evo_optimizer(
        args.phase1_optimizer, args.phase1_pop_size, problem, args, de_param_control
    )
    optimizer.setup(problem, termination=NoTermination(), verbose=True)


    pop_init = None
    if args.phase1_pop_init is not None:
        if args.phase1_pop_init == "normal":
            pop_init = rng.normal(0.0, args.phase1_pop_init_std, size=(args.phase1_pop_size, num_dims))
        elif args.phase1_pop_init == "uniform":
            pop_init = rng.uniform(args.phase1_pop_init_lb, args.phase1_pop_init_ub, size=(args.phase1_pop_size, num_dims))
        else:
            raise ValueError(f"Invalid population initialization: {args.phase1_pop_init}")
    ...
    # Phase 1: Full-space evolutionary optimization (DE or PSO)

    for iteration in range(1, args.phase1_num_iterations + 1):
        if args.num_fe is not None and fe_count >= args.num_fe:
            print(
                f"Stopping phase 1: function evaluation budget reached (num_fe={args.num_fe}, fe_count={fe_count})."
            )
            break
        step += 1
        ...
        if optimizer.pop is not None:
            ...
            for i in range(len(pop_X)):
                ...
                with torch.no_grad():
                    ...
                    output = phase1_adapter.model(x)
                    loss += criterion(output, y)
                    l2 += _param_l2(phase1_adapter.model)
                    F += (loss + l2_coef * l2)
        ...
        if args.phase1_optimizer == "de" and args.de_control:
            ...
            if s:
                print(f"[phase1 iter {iteration}] de_control (best pop member): {s}")

        wandb.log({"phase1/best/f": best_F, "phase1/best/loss": best_loss, "phase1/best/l2_reg": best_l2,
                    "evo/best/f": best_F, "evo/best/loss": best_loss, "evo/best/l2_reg": best_l2}, step=step)

        top10_idx = np.argsort(result.pop.get("F"))[:10]
        center_X = np.mean(result.pop.get("X")[top10_idx], axis=0)
        phase1_adapter.apply(theta_0, center_X, alpha=args.alpha, device=device)
        center_loss = 0
        center_F = 0
        center_l2 = 0
        with torch.no_grad():
            ...
            center_l2 = _param_l2(phase1_adapter.model)
            center_F = (center_loss + l2_coef * center_l2)
        wandb.log({"phase1/center/f": center_F, "phase1/center/loss": center_loss, "phase1/center/l2_reg": center_l2,
                    "evo/center/f": center_F, "evo/center/loss": center_loss, "evo/center/l2_reg": center_l2}, step=step)

        if center_F > best_F:
            phase1_adapter.apply(theta_0, best_X, alpha=args.alpha, device=device)

        if args.test_interval is not None and iteration % args.test_interval == 0:
            test_metrics = evaluate_model_on_test(model=phase1_adapter.model, data_loader=test_loader, device=device)
                ...
                torch.save(
                    {
                        ...
                        "step": step,
                        "adapter": "full",
                        "model_state_dict": phase1_adapter.model.state_dict(),
                        "adapter_state": adapter_checkpoint_dict(phase1_adapter),
                        "test_metrics": dict(best_test_metrics),
                    },
                    ckpt_path,
                )
                print(
                    f"[phase1] New best test (by top1) — {', '.join(f'{k}={v:.4f}' for k, v in best_test_metrics.items())} — saved to {ckpt_path}"
                )
    ...
    if not args.disable_wandb:
        ...
        ckpt_path,
        metadata={**best_test_metrics, "phase": 1, "phase_iteration": iteration, "step": step},
                    )
    if args.num_fe is not None and fe_count >= args.num_fe:
        print(
            f"Skipping phase 2: function evaluation budget reached after phase 1 (num_fe={args.num_fe}, fe_count={fe_count})."
        )
    else:
        phase2_adapter = get_adapter(args.adapter, model=phase1_adapter.model, args=args)
        num_dims = phase2_adapter.num_dims
    
        phase2_xl = -1.0 if args.phase2_pop_init_lb is None else float(args.phase2_pop_init_lb)
        phase2_xu = 1.0 if args.phase2_pop_init_ub is None else float(args.phase2_pop_init_ub)
        problem = Problem(
            n_var=num_dims,
            n_obj=1,
            n_constr=0,
        ...
        optimizer = _build_evo_optimizer(
            args.phase2_optimizer, args.phase2_pop_size, problem, args, de_param_control
        )
        optimizer.setup(problem, termination=NoTermination(), verbose=True)
    
    
        pop_init = phase2_adapter.init_pop(type=args.phase2_pop_init, pop_size=args.phase2_pop_size, sigma=args.phase2_pop_init_std, lb=args.phase2_pop_init_lb, ub=args.phase2_pop_init_ub)
    
        # train_loader_iterator = itertools.cycle(train_loader)
    
        # Phase 2: Subspace evolutionary optimization (DE or PSO)
    
        for iteration in range(1, args.phase2_num_iterations + 1):
            if args.num_fe is not None and fe_count >= args.num_fe:
                print(
                    f"Stopping phase 2: function evaluation budget reached (num_fe={args.num_fe}, fe_count={fe_count})."
                )
                break
            step += 1
            ...
            if optimizer.pop is not None:
                ...
                for i in range(len(pop_X)):
                    ...
                    with torch.no_grad():
                        x, y = curr_batch
                        x, y = x.to(device), y.to(device)
                        output = phase2_adapter.model(x)
                        loss += criterion(output, y)
                        l2 += _param_l2(phase2_adapter.model)
                        F += (loss + l2_coef * l2)
            ...
            offspring = optimizer.ask()
            if iteration == 1 and pop_init is not None:
                print(f"[phase2] Setting initial population: {np.asarray(pop_init).shape}")
                print(
                    f"[phase2] Initial population min: {np.asarray(pop_init).min()}, max: {np.asarray(pop_init).max()}"
                )
                offspring.set("X", np.asarray(pop_init, dtype=np.float64).copy())
            ...
            best_loss = result.opt.get("loss")[best_idx, 0]
            best_F = result.opt.get("F")[best_idx, 0]
            if args.phase2_optimizer == "de" and args.de_control:
                best_pop_idx = int(np.argmin(np.asarray(result.pop.get("F")).reshape(-1)))
                s = _de_control_str_from_individual(result.pop[best_pop_idx])
                if s:
                    print(f"[phase2 iter {iteration}] de_control (best pop member): {s}")
    
            wandb.log({"phase2/best/f": best_F, "phase2/best/loss": best_loss, "phase2/best/l2_reg": best_l2,
                        "evo/best/f": best_F, "evo/best/loss": best_loss, "evo/best/l2_reg": best_l2}, step=step)
    
            top10_idx = np.argsort(result.pop.get("F"))[:10]
            center_X = np.mean(result.pop.get("X")[top10_idx], axis=0)
            phase2_adapter.apply(theta_0, center_X, alpha=args.alpha, device=device)
            center_loss = 0
            center_F = 0
            center_l2 = 0
            with torch.no_grad():
                ...
                output = phase2_adapter.model(x)
                center_loss = criterion(output, y)
                center_l2 = _param_l2(phase2_adapter.model)
                center_F = (center_loss + l2_coef * center_l2)
            wandb.log({"phase2/center/f": center_F, "phase2/center/loss": center_loss, "phase2/center/l2_reg": center_l2,
                        "evo/center/f": center_F, "evo/center/loss": center_loss, "evo/center/l2_reg": center_l2}, step=step)
    
            if center_F > best_F:
                phase2_adapter.apply(theta_0, best_X, alpha=args.alpha, device=device)
    
            if args.test_interval is not None and iteration % args.test_interval == 0:
                test_metrics = evaluate_model_on_test(model=phase2_adapter.model, data_loader=test_loader, device=device)
                    ...
                    torch.save(
                        {
                            ...
                            "model_state_dict": phase2_adapter.model.state_dict(),
                            "adapter_state": adapter_checkpoint_dict(phase2_adapter),
                            "test_metrics": dict(best_test_metrics),
                        },
                        ckpt_path,
                    )
                    print(
                        f"[phase2] New best test (by top1) — {', '.join(f'{k}={v:.4f}' for k, v in best_test_metrics.items())} — saved to {ckpt_path}"
                    )
            ...
            wandb.log(
                {
                    ...
                    "evo/iteration": args.phase1_num_iterations + iteration,
                    "evo/function_evaluation": fe_count,
                },
                step=step,
            )
        if not args.disable_wandb:
            ...
            log_checkpoint_wandb(
                ckpt_path,
                metadata={**best_test_metrics, "phase": 2, "phase_iteration": iteration, "step": step},
            )
    if not args.disable_wandb:
        wandb.finish()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        choices=['de', 'pso'],
        help='Default evolutionary optimizer for both phases when phase-specific optimizers are omitted',
        required=True,
    )
    parser.add_argument(
        '--phase1_optimizer',
        ...
        default=None,
        choices=['de', 'pso'],
        help='Phase 1 optimizer (default: --optimizer)',
    )
    parser.add_argument(
        '--phase2_optimizer',
        type=str,
        default=None,
        choices=['de', 'pso'],
        help='Phase 2 optimizer (default: --optimizer)',
    )
    parser.add_argument('--pop_size', type=int, default=50, help='Default population size when phase pop sizes are omitted')
    parser.add_argument('--phase1_num_iterations', type=int, default=None,
                       help='Number of iterations for phase 1')
    parser.add_argument('--phase2_num_iterations', type=int, default=None,
                       help='Number of iterations for phase 2')
    parser.add_argument(
        '--num_fe',
        type=int,
        default=None,
        help='Max fitness evaluations (population re-eval + offspring per iteration, phase 1 + phase 2); stop at the start of the next iteration when the counter has reached this. Phase 2 is skipped if the budget is already exhausted after phase 1. None disables.',
    )
    parser.add_argument('--phase1_pop_size', type=int, default=None,
                       help='Phase 1 population size (defaults to --pop_size)')
    parser.add_argument('--phase2_pop_size', type=int, default=None,
                       help='Phase 2 population size (defaults to --pop_size)')
    parser.add_argument('--phase1_pop_init', type=str, default=None,
                       help='Initial distribution for phase 1')
    parser.add_argument('--phase2_pop_init', type=str, default=None,
                       help='Initial distribution for phase 2')
    parser.add_argument('--phase1_pop_init_lb', type=float, default=None,
                       help='Initial lower bound for population initialization with uniform distribution')
    parser.add_argument('--phase1_pop_init_ub', type=float, default=None,
                       help='Initial upper bound for population initialization with uniform distribution')
    parser.add_argument('--phase1_pop_init_std', type=float, default=None,
                       help='Initial standard deviation for population initialization with normal distribution')
    parser.add_argument('--phase2_pop_init_lb', type=float, default=None,
                       help='Initial lower bound for population initialization with uniform distribution')
    parser.add_argument('--phase2_pop_init_ub', type=float, default=None,
                       help='Initial upper bound for population initialization with uniform distribution')
    parser.add_argument('--phase2_pop_init_std', type=float, default=None,
                       help='Initial standard deviation for population initialization with normal distribution')
    ...
    parser.add_argument('--de_jitter', action='store_true', default=None,
                       help='Jitter factor for DE optimizer')
    parser.add_argument('--de_mut_prob', type=float, default=None,
    ...
    # ============================================================================
    parser.add_argument('--adapter', type=str, default='randproj',
                       help='Adapter strategy (e.g., FullSpace, LoRA, etc.)', required=True)
    ...
    if args.phase1_pop_size is None:
        args.phase1_pop_size = args.pop_size
    if args.phase2_pop_size is None:
        args.phase2_pop_size = args.pop_size
    if args.phase1_optimizer is None:
        args.phase1_optimizer = args.optimizer
    if args.phase2_optimizer is None:
        args.phase2_optimizer = args.optimizer
    main(args)
