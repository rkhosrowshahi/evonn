    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if optimizer.pop is not None and iteration > 1:
            ...
            for i in range(len(pop_X)):
                ...
                # pop_F[i, 0] = pop_F[i, 0] / (1 - 0.1**iteration)
                # pop_F[i, 0] = F
                fe_count += 1
        ...
        # print(f"Iteration {iteration} - Offspring F raw: {np.sort(offspring_F.flatten())}")
        if iteration > 1:
            offspring_F[:, 0] = 0.5 * momentum + 0.5 * offspring_F[:, 0]
            # offspring_F[:, 0] = offspring_F[:, 0] / (1 - 0.5**1)
        # print(f"Iteration {iteration} - Momentum: {momentum:.4f} - Offspring F weighted ema: {np.sort(offspring_F.flatten())}")
        ...
        if iteration >= 1:
            ...
            # momentum = args.fitness_ema_beta * pop_F + (1 - args.fitness_ema_beta) * offspring_F.mean()
            # momentum = pop_F.mean() # Worked but stopped to improve at 20% accuracy, meaning it was not effective and lying about the estimated fitness.
            if iteration > 1:
                momentum = 0.9 * momentum + 0.1 * pop_F[topk_pop_F].mean()
            else:
                momentum = pop_F[topk_pop_F].mean()
            # momentum = pop_F[topk_pop_F].mean()

            # momentum = momentum / (1 - 0.9**iteration)
