...
from pymoo.algorithms.soo.nonconvex.de import Variant as DEVariant
...
from pymoo.util.display.single import AverageConstraintViolation, MinimumConstraintViolation
...
def _de_control_str_from_individual(ind) -> str:
    ...
    if selection is not None:
        s = (
            ...
            else str(np.asarray(selection).ravel()[0])
        )
...
def main(args):
    ...
    # Load data and model
    train_loader, val_loader, test_loader, num_classes, input_size = create_dataset(
        args
    )
    ...
    if args.pop_init is not None:
        if args.pop_init == "normal":
            pop_init = rng.normal(
                0.0, args.pop_init_std, size=(args.pop_size, num_dims)
            )
        elif args.pop_init == "uniform":
            pop_init = rng.uniform(
                args.pop_init_lb, args.pop_init_ub, size=(args.pop_size, num_dims)
            )
    ...
    if args.optimizer == "de":
        de_param_control = (
            EvolutionaryParameterControl if args.de_control else NoParameterControl
        )
        ...
        class DEOutput(Output):
            ...
            def __init__(self):
                ...
                self.best_momentum = Column(name="best_momentum")
                self.best_l2 = Column(name="best_l2")
                self.best_loss = Column(name="best_loss")
                self.best_age = Column(name="best_age")
            ...
            def initialize(self, algorithm):
                ...
                self.columns += [self.f_avg, self.f_min, self.best_loss, self.best_l2, self.best_momentum, self.best_age]
            ...
            def update(self, algorithm):
                ...
                if feas.sum() > 0:
                    self.f_avg.set(f"{f[feas].mean():.4f}")
                else:
                    self.f_avg.set(None)

                opt = algorithm.opt[0]
                
                loss, l2, momentum, age = opt.get("loss", "l2", "momentum", "age")
                self.best_loss.set(f"{loss[0]:.4f}")
                self.best_l2.set(f"{l2[0]:.4f}")
                self.best_momentum.set(f"{momentum[0]:.4f}")
                self.best_age.set(f"{int(age[0])}")

                if opt.feas:
                    self.f_min.set(f"{opt.f:.4f}")
    ...
    # Full-space evolutionary optimization
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        best_parent_momentum = None
        best_parent_batch_F = None
        ...
        if optimizer.pop is not None and iteration > 1:
            ...
            pop_momentum = optimizer.pop.get("momentum")
            pop_batch_F = np.zeros((len(pop_X), 1))

            if pop_momentum is None:
                # Should not happen if initialized properly in offspring, but fallback
                pop_momentum = pop_F.copy()
            ...
            for i in range(len(pop_X)):
                ...
                pop_batch_F[i, 0] = F
                pop_momentum[i, 0] = (
                    args.fitness_ema_beta * pop_momentum[i, 0]
                    + (1 - args.fitness_ema_beta) * F
                )
                pop_F[i, 0] = pop_momentum[i, 0]
                fe_count += 1

            best_parent_idx = int(np.argmin(np.asarray(pop_momentum).reshape(-1)))
            best_parent_momentum = float(pop_momentum[best_parent_idx, 0])
            best_parent_batch_F = float(pop_batch_F[best_parent_idx, 0])

            optimizer.pop.set("F", pop_F)
            optimizer.pop.set("loss", pop_loss)
            optimizer.pop.set("momentum", pop_momentum)
        ...
        offspring_momentum = np.zeros((len(offspring_X), 1))
        offspring_age = np.ones((len(offspring_X), 1)) * iteration
        ...
        if args.test_interval is not None and iteration % args.test_interval == 0:
            test_metrics = evaluate_model_on_test(
                model=param_adapter.model, data_loader=test_loader, device=device
            )
            ...
            if (
                ...
                or test_metrics["top1"] > best_test_metrics["top1"]
            ):
                ...
                print(
                    f"New best test (by top1) — {', '.join(f'{k}={v:.4f}' for k, v in best_test_metrics.items())}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--val_split", type=float, default=0.0, help="Validation split for training"
    )
    parser.add_argument(
        "--sampler", type=str, default=None, help="Sampler for training"
    )
    parser.add_argument(
        "--criterion",
        type=str,
        default="ce",
        choices=["ce", "cm", "mse", "f1", "soft_f1", "ce_sf1"],
    ...
    parser.add_argument(
        "--f1_beta", type=float, default=None, help="Beta for soft F1 score error"
    )
    parser.add_argument(
        "--f1_learnable_temperature",
        type=bool,
        default=False,
    ...
    parser.add_argument(
        "--momentum", type=float, default=None, help="Momentum factor for SGD optimizer"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Initial distribution (normal or uniform)",
    )
    parser.add_argument(
        "--pop_init_lb",
        type=float,
        default=None,
        help="Lower bound for uniform pop_init",
    )
    parser.add_argument(
        "--pop_init_ub",
        type=float,
        default=None,
        help="Upper bound for uniform pop_init",
    )
    parser.add_argument(
        "--pop_init_std",
        type=float,
        default=None,
        help="Standard deviation for normal pop_init",
    )
    # DE optimizer configuration
    parser.add_argument(
        "--de_cr_rate", type=float, default=None, help="Crossover rate for DE optimizer"
    )
    parser.add_argument(
        "--de_mut_rate", type=float, default=None, help="Mutation rate for DE optimizer"
    )
    parser.add_argument(
        "--de_selection",
        type=str,
    ...
    parser.add_argument(
        "--es_lr", type=float, default=None, help="Learning rate for ES optimizer"
    )
    parser.add_argument(
        "--es_optimizer",
        type=str,
    ...
    parser.add_argument(
        "--disable_wandb", action="store_true", help="Disable wandb logging completely"
    )
    parser.add_argument(
        "--note",
        type=str,
